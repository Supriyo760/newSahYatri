...
export default function Onboarding() {
      ...
      try {
        ...
        if (profileData.data) {
          ...
          setProfileForm(prev => ({
            ...
            agreeableness: p.agreeableness ?? prev.agreeableness,
            neuroticism: p.neuroticism ?? prev.neuroticism,
            ...
            interests: p.interests?.join(', ') || prev.interests,
          }));
        }
        ...
        if (medData.data) {
          ...
          setMedicalForm(prev => ({
            ...prev,
            bloodType: m.bloodType || prev.bloodType,
            conditions: m.conditions?.length ? m.conditions : prev.conditions,
            medications: m.medications?.length ? m.medications : prev.medications,
            allergies: m.allergies?.length ? m.allergies : prev.allergies,
            emergencyContacts: m.emergencyContacts?.length ? m.emergencyContacts : prev.emergencyContacts,
            firstAidNotes: m.firstAidNotes || prev.firstAidNotes,
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(resData.error?.message || 'Failed to upload avatar');
      }
  ...
  const nextStep = () => {
    ...
    if (step === 1) {
      ...
      if (!profileForm.interests) {
        setMessage('Error: Please add at least one shared interest.');
        return;
      }
      setStep(3);
    }
  ...
  const prevStep = () => {
    setMessage('');
    setStep(step > 1 ? (step - 1) as 1 | 2 | 3 : 1);
  };
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(resData.error?.message || 'Failed to save profile');
      ...
      res = await fetch('/api/medical/profile', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!res.ok) throw new Error(resData.error?.message || 'Failed to save medical profile');
    ...
    <div className="min-h-screen bg-[#fbf9f4] text-[#1b1c19] flex flex-col font-sans pb-24">
      ...
      <main className="flex-1 pt-24 max-w-4xl w-full mx-auto px-6">
        ...
        <div className="text-center mb-10">
          <span className="font-journal-label text-[#8f361d] tracking-widest block mb-2">
            STEP {step} OF 3
          </span>
          <h1 className="font-journal-headline text-4xl md:text-5xl text-[#1b1c19]">
            {step === 1 && "Who is the traveler?"}
            {step === 2 && "Vibe & Preferences"}
            {step === 3 && "Secure Medical Card"}
          </h1>
        ...
        <div className="bg-[#f0eee9] p-8 rounded-2xl shadow-tactile space-y-6">
              ...
              <div className="flex flex-col items-center justify-center py-6 border-b border-[#ddc0b9]/30">
                ...
                <div className="relative group">
                  <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-[#8f361d] shadow-lg relative bg-[#e4e2dd] transition-all hover:brightness-95">
                    <img
                      src={avatarUrl || 'https://lh3.googleusercontent.com/aida-public/AB6AXuCELUwqFW8Qdkh2CfTBkR3Oxcv-Arc2HtCNQsPYI0DNAF7nE_lrIpByFVZHnHdB3BapO-Fu6X3n1X6DE6FgdF9nO8lyjtmwXr85-X9yCDkDNAF7nE_lrIpByFVZHnHdB3BapO-Fu6X3n1X6DE6FgdF9nO8lyjtmwXr85-X9yCDkDNAF7nE_lrIpByFVZHnHdB3BapO-Fu6X3n1X6DE6FgdF9nO8lyjtmwXr85-X9yCDkDMgEir4hBW9WvPSk7ApRh004HM3nghn66MReNj6AEmt4SJTLV67HnKMONXxgaafAFp0M9zPc65lui_Yptfp924FqWLm4elACNLhZvy4AbuUWRBXAKpIC3EqSGZupIiwh-8XOHt3sZMBLyLOWdWqBlDCLqBNAo0yelsxk'}
                      alt="Traveler avatar"
          ...
          {step === 3 && (
            <div className="space-y-6 animate-fade-in bg-[#fdb55c]/10 border border-[#fdb55c]/20 p-6 rounded-2xl">
          ...
          {/* Wizard Controls */}
          <div className="flex gap-4 pt-6 border-t border-[#ddc0b9]/40">
            {step > 1 && (
              <button
                type="button"
                onClick={prevStep}
                disabled={loading}
                className="w-1/3 bg-transparent border border-[#ddc0b9] text-[#89726c] py-3.5 rounded-full font-journal-label text-xs tracking-widest uppercase hover:bg-[#ddc0b9]/20 transition-colors"
              >
                BACK
              </button>
            )}
            ...
            {step < 3 ? (
              <button
                ...
                onClick={nextStep}
                disabled={loading}
                className="flex-1 bg-[#8f361d] text-white py-3.5 rounded-full font-journal-label text-xs tracking-widest uppercase hover:bg-[#af4d32] transition-colors shadow-tactile"
              >
              ...
              <button
                ...
                disabled={loading}
                className="flex-1 bg-[#283b31] text-[#fbf9f4] py-3.5 rounded-full font-journal-label text-xs tracking-widest uppercase hover:bg-[#344b3f] transition-colors shadow-tactile"
              >
                {loading ? 'SAVING DATA...' : 'COMPLETE ONBOARDING'}
              </button>
            )}
          </div>
        </div>
