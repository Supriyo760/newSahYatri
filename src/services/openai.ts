...
export async function generateItinerary(params: GenerationParams): Promise<GeneratedItinerary> {
  ...
  const isPlaceholder = !apiKey || apiKey === 'sk-placeholder' || apiKey.startsWith('sk-Placeholder') || apiKey.includes('placeholder');

  if (isPlaceholder) {
    console.log('Using fallback itinerary generator because OpenAI API key is a placeholder');
